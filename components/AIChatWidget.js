...
const WELCOME = "What's your current biggest bottleneck — finding leads, following up, or closing them?";
...
export default function AIChatWidget() {
  ...
  const { messages, input, setInput, handleSubmit, append, status, error } = useChat({
    api: '/api/chat',
    body: { agentType: 'lead_gen' },
    id: 'staffai-agent',
    onError: (err) => {
      console.error('[StaffAI Widget]', err);
      setErrorMsg('Connection issue — please try again in a moment.');
  ...
  // Show lead form when AI asks for name/email
  useEffect(() => {
    if (!leadCaptured && messages.length > 0) {
      ...
      if (last.role === 'assistant') {
        ...
        if (
          ...
          text.includes('best email') ||
          text.includes('your email') ||
          text.includes('grab your email')
        ) {
  ...
  // Extract text from either content string or parts array (handles both AI SDK v3 and v4+)
    ...
    <div style={{ position: 'fixed', bottom: '1.5rem', right: '1.5rem', zIndex: 9999, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      {isOpen && (
        <div style={{
          ...
          {/* HEADER */}
          <div style={{
            padding: '0.9rem 1.1rem', background: '#13131e',
          ...
          <div style={{
            ...
            <div style={{ alignSelf: 'flex-start', maxWidth: '83%' }}>
              ...
              <div style={{
                background: '#1c1c2a', color: '#f9fafb',
            ...
            {messages.map(m => {
              ...
              return (
                <div key={m.id} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '83%' }}>
                  ...
                  <div style={{
                    background: m.role === 'user' ? 'linear-gradient(135deg, #2563eb, #1d4ed8)' : '#1c1c2a',
                    color: '#f9fafb',
                    padding: '0.65rem 0.95rem',
            ...
            {showLeadForm && !leadCaptured && (
              <div style={{ alignSelf: 'flex-start', width: '90%' }}>
                <form onSubmit={handleLeadFormSubmit} style={{
                  background: '#1c1c2a', border: '1px solid rgba(37,99,235,0.35)',
                  borderRadius: '14px', padding: '0.9rem',
                  display: 'flex', flexDirection: 'column', gap: '0.55rem'
                }}>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Your details</div>
                  <input
                    type="text" placeholder="First name"
                    value={leadFormData.name}
                    onChange={e => setLeadFormData(p => ({ ...p, name: e.target.value }))}
                    autoFocus
                    style={{ padding: '0.6rem 0.8rem', background: '#252535', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#f9fafb', fontSize: '0.875rem', outline: 'none', width: '100%', boxSizing: 'border-box' }}
                  />
                  <input
                    ...
                    style={{ padding: '0.6rem 0.8rem', background: '#252535', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#f9fafb', fontSize: '0.875rem', outline: 'none', width: '100%', boxSizing: 'border-box' }}
                  />
                  <button type="submit" disabled={!leadFormData.name.trim() || !leadFormData.email.trim()}
                    style={{ padding: '0.6rem', background: 'linear-gradient(135deg, #2563eb, #1d4ed8)', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '0.875rem', cursor: (!leadFormData.name.trim() || !leadFormData.email.trim()) ? 'not-allowed' : 'pointer', opacity: (!leadFormData.name.trim() || !leadFormData.email.trim()) ? 0.45 : 1 }}>
                    Continue →
                  </button>
                </form>
              </div>
          ...
          <form onSubmit={onSubmit} style={{
            padding: '0.8rem 0.9rem', background: '#13131e',
            ...
            <input
              ...
              style={{
                ...
                borderRadius: '24px', color: '#f9fafb',
                fontSize: '0.875rem', outline: 'none', caretColor: '#f9fafb'
              }}
            />
