...
Return a final summary of what you accomplished.
    ...
    console.log(`[Engine] Handing off to LLM (Model: openrouter/${modelId})...`);
    ...
    const hasTools = Object.keys(permittedTools).length > 0;
    const { text, usage, toolCalls, toolResults } = await generateText({
      model,
      system: systemPrompt,
      messages: llmMessages,
      tools: hasTools ? permittedTools : undefined,
      toolChoice: hasTools ? 'auto' : undefined,
      maxSteps: 5, // Prevent infinite loops
    });
    console.log(`[Engine] Raw toolCalls: ${JSON.stringify(toolCalls)}`);
