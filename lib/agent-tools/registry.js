...
import {
  outscraperSearch, outscraperConfigured,
  verifyEmails, verificationConfigured,
  sendBulkEmail, bulkSendingConfigured,
} from './providers';
...
export const TOOLS = {
  ...
  /**
   * Lead lists at volume. Dark until OUTSCRAPER_API_KEY exists, at which point
   * the Lead Generation Specialist gains it without a deploy. Its research by
   * hand is the quality path and stays the default; this is the volume path,
   * and its records are unverified until they pass through verify_emails.
   */
  find_businesses: {
    roles: ['Lead Generation Specialist', 'Marketing Manager'],
    available: outscraperConfigured,
    async run(input) {
      const { query, limit } = input;
      const rows = await outscraperSearch({ query, limit });
      return {
        count: rows.length,
        businesses: rows,
        note: 'These addresses are unverified. Run verify_emails before any of them is used for outreach.',
      };
    },
  },

  /**
   * Verification before sending. Bounces are what burn a sending domain, so an
   * unverified scraped address must never reach a campaign.
   */
  verify_emails: {
    roles: ['Lead Generation Specialist', 'Sales Representative', 'Marketing Manager'],
    available: verificationConfigured,
    async run(input) {
      const emails = Array.isArray(input.emails) ? input.emails : [input.email].filter(Boolean);
      if (!emails.length) throw new Error('emails is required, as an array of addresses');
      const results = await verifyEmails(emails);
      return {
        checked: results.length,
        sendable: results.filter(r => r.sendable).map(r => r.email),
        risky: results.filter(r => r.risky).map(r => r.email),
        rejected: results.filter(r => !r.sendable && !r.risky).map(r => r.email),
        results,
      };
    },
  },
  ...
  /**
   * Campaign sending, kept away from product email on purpose. Resend carries
   * receipts, password resets and notifications for every customer; a campaign
   * that damages a reputation must damage only the customer that sent it.
   */
  send_campaign_email: {
    roles: ['Sales Representative', 'Marketing Manager', 'Marketing Specialist'],
    available: bulkSendingConfigured,
    async run(input, { employee, organization, db }) {
      const { to, subject, body } = input;
      const recipients = Array.isArray(to) ? to : [to].filter(Boolean);
      if (!recipients.length) throw new Error('to is required, as an array of addresses');
      if (recipients.length > 50) throw new Error('send at most 50 at a time, so a mistake stays small');
      ...
      await db.from('staffai_events').insert({
        ...
        source: 'agent-tools',
        status: 'ok',
        data: { employee_id: employee.id, recipients: recipients.length, subject },
      });
      return { sent: recipients.length, raw: result };
    },
  },
