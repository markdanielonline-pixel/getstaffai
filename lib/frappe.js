...
async function getFrappeSecrets() {
  if (process.env.FRAPPE_ADMIN_PASSWORD && process.env.FRAPPE_PROVISION_WEBHOOK_SECRET) {
    return {
      adminPassword: process.env.FRAPPE_ADMIN_PASSWORD,
      webhookSecret: process.env.FRAPPE_PROVISION_WEBHOOK_SECRET,
    };
  }
  if (!process.env.INFISICAL_CLIENT_ID || !process.env.INFISICAL_CLIENT_SECRET || !process.env.INFISICAL_PROJECT_ID) {
    throw new Error('Frappe credentials are not configured');
  }
...
export async function createFrappeUser(orgId, siteDomain, employeeName, email, role) {
  try {
    ...
    const baseUrl = process.env.FRAPPE_INTERNAL_BASE_URL || `http://${siteDomain}`;
    const apiUrl = `${baseUrl}/api/resource/User`;
    ...
    const response = await fetch(apiUrl, {
      ...
      headers: {
        ...
        ...(process.env.FRAPPE_INTERNAL_BASE_URL ? { Host: siteDomain } : {}),
      },
      body: JSON.stringify({
        email: email,
