...
export async function awaitProvisionAgentOperational(employeeId, orgId, options = {}) {
  ...
  // Only a reading that stayed non-operational for the whole budget is worth
  // writing down as a lifecycle demotion.
  return operational(runtime) ? runtime : syncProvisionAgent(employeeId, orgId);
}
...
export async function decommissionAgentRuntime(employeeId, orgId) {
  ...
  if (employee.provision_agent_id) {
    // team_external_id is the tenant binding: Provision refuses to delete an
    // agent that does not belong to the team this organization maps to.
    await request(`/api/integrations/staffai/agents/${encodeURIComponent(employeeId)}`, {
      method: 'DELETE', timeoutMs: 90_000, body: { team_external_id: organization.provision_team_id },
    });
  }
  data(await supabase.from('employees').update({
