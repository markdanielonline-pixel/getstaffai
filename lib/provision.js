...
export async function provisionTeamRuntime(organization) {
  ...
  if (runtime.external_id !== organization.id || (organization.provision_team_id && runtime.id !== organization.provision_team_id)) {
    throw new Error('Provision team ownership mismatch');
  }
...
export async function syncProvisionTeam(organizationId) {
  const runtime = await request(`/api/integrations/staffai/teams/${encodeURIComponent(organizationId)}`);
  const supabase = adminClient();
  ...
  if (runtime.external_id !== organizationId || (organization.provision_team_id && organization.provision_team_id !== runtime.id)) throw new Error('Provision team ownership mismatch');
  data(await supabase.from('organizations').update({
    provision_team_id: runtime.id,
    provision_connection_status: runtime.runtime_status,
    provision_last_synced_at: new Date().toISOString(),
...
export async function provisionAgentRuntime(orgId, employeeId, roleTemplate, onboardingAnswers = {}) {
  ...
  const runtime = await request(`/api/integrations/staffai/teams/${encodeURIComponent(organization.provision_team_id)}/agents`, {
    ...
    body: {
      ...
      model: process.env.PROVISION_DEFAULT_MODEL || 'z-ai/glm-4.7',
  ...
  if (runtime.external_id !== employee.id || runtime.team_id !== organization.provision_team_id
    || (employee.provision_agent_id && employee.provision_agent_id !== runtime.id)) throw new Error('Provision agent ownership mismatch');
...
export async function syncProvisionAgent(employeeId, orgId) {
  ...
  const runtime = await request(`/api/integrations/staffai/agents/${encodeURIComponent(employeeId)}`);
