...
export async function provisionDispatcher(provisionTeamId) {
  return request(`/api/integrations/staffai/teams/${encodeURIComponent(provisionTeamId)}/dispatcher`, {
    method: 'POST', body: {},
  });
}
...
export async function provisionAgentRuntime(orgId, employeeId, roleTemplate, onboardingAnswers = {}) {
  ...
  const employee = data(await supabase.from('employees')
    .select('id, name, title, role, manager_id').eq('id', employeeId).eq('org_id', orgId).single(), 'Load employee');
...
export async function syncProvisionAgent(employeeId) {
  ...
  data(await supabase.from('employees').update({
    ...
    provision_last_synced_at: new Date().toISOString(),
    provision_error: null,
    ...(runtime.status === 'active' ? { status: 'active' } : {}),
  }).eq('id', employeeId), 'Synchronize Provision agent status');
  return runtime;
}
