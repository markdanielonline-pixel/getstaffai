...
import { untriagedReports } from './reports';
 ...
 */
const warnings = {
  // What customers and visitors are telling us. No health check can produce
  // this signal, because the checks only know what we thought to check.
  async userReports() {
    const { count, reports } = await untriagedReports();
    if (count) {
      return { ok: false, detail: count + ' untriaged report(s): ' + reports.slice(0, 3).map(r => r.reference + ' ' + r.symptom.slice(0, 90)).join(' | ') };
    }
    return { ok: true };
  },
