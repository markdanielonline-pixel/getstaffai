...
export async function hireAdditionalEmployee({ ceo, roleKey, idempotencyKey }) {
  ...
  let employee;
  try {
    employee = await instantiateEmployee({
      orgId: ceo.org_id,
...
function humanHiringError(error, role) {
  ...
  if (/employees_org_ceo_role_unique|duplicate key/i.test(text)) {
    // Kept as a safety net only. Multiple employees per role are now allowed;
    // if this ever fires again it means a genuine duplicate, not a policy.
    return new Error(`That hire collided with an existing employee record. Please try again.`);
  }
