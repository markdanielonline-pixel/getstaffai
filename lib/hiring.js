...
export const HIREABLE_ROLES = Object.entries(BILLING_CATALOG)
  ...
  .map(([key, product]) => ({ key, name: product.name, monthly: product.monthly, annual: product.annual }));
...
export async function hireAdditionalEmployee({ ceo, roleKey, idempotencyKey }) {
  ...
  const employee = await instantiateEmployee({
    ...
    roleTemplate: {
      ...
      required_tools: [],
      default_kpis: [],
      base_prompt_template: `You are the ${role.name} for ${ceo.company_name || organization.name}.
Staff AI owns approvals and autonomy. Follow its authorized instructions; never claim an unperformed action.
Company context: ${JSON.stringify({ description: ceo.business_description })}`,
    },
