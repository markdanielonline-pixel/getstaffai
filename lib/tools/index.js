...
export const send_email = ({ orgId, employeeId, taskId }) => tool({
  ...
  execute: async ({ to, subject, body }) => {
    ...
    if (!process.env.RESEND_API_KEY) {
      ...
      return { success: false, error: 'RESEND_API_KEY is missing. Action aborted.' };
    }
    ...
    try {
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Staff AI <ai@getstaffai.com>',
          to: to,
          subject: subject,
          html: body
        })
...
export const send_sms = ({ orgId, employeeId, taskId }) => tool({
  ...
  execute: async ({ recipient, body }) => {
    ...
    if (!process.env.TELNYX_API_KEY) {
      console.error("TELNYX_API_KEY missing. Cannot send SMS.");
      return { success: false, error: 'TELNYX_API_KEY is missing. Action aborted.' };
    }
