...
export async function sendReminderEmail({ email, name, topic }) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `<p>Hi ${escapeHtml(name || 'there')},</p><p>This is your StaffAI reminder:</p><p><strong>${escapeHtml(topic)}</strong></p>`,
    }),
  });

  if (!response.ok) throw new Error(`Reminder email delivery failed with status ${response.status}`);
...
export async function sendEAOnboardingEmail(email, firstName, eaName = null) {
  ...
  if (!resendKey) {
    console.error("[Resend] API Key missing. Cannot send EA onboarding email.");
