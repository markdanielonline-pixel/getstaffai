...
export async function recordSeatOpened({ ceo, employeeId, roleKey, roleName, monthlyCents, charge }) {
  ...
  // Checked, not fired and forgotten. The first version of this insert was
  // rejected every time, because seat_billing_records still carried NOT NULL
  // columns from an older pricing model and billing_type had no 'comped' value.
  // Nothing checked the result, so the ledger stayed empty and looked fine.
  const { error } = await db.from('seat_billing_records').insert({
    ceo_id: ceo.id,
  ...
  if (error) {
    // A seat that is charged but unrecorded is a billing discrepancy, so this
    // is recorded where the monitor can see it rather than only in a log line.
    console.error('[billing.seats] could not record seat', employeeId, error.message);
    await db.from('staffai_events').insert({
      event_type: 'billing.seat_record_failed',
      source: 'billing.seats',
      status: 'recorded',
      data: { employeeId, roleKey, billed: charge?.billed === true, subscriptionItemId: charge?.subscriptionItemId || null, error: error.message },
    }).then(() => null, () => null);
  }

  // The seat fee belongs on the employee too, because that is what the roster
  // and any future invoice reconciliation read.
  const employeeUpdate = { seat_fee_cents: charge?.comped ? 0 : monthlyCents };
  if (charge?.subscriptionItemId) employeeUpdate.stripe_subscription_item_id = charge.subscriptionItemId;
  await db.from('employees').update(employeeUpdate).eq('id', employeeId);
}
